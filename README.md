...
- Weekly page optimization: every Monday morning, the site refreshes the page-level keyword plan for the home, service, Africa, Amsterdam, Munich, and Stockholm landing pages.
...
- `src/pages/api/cron/seo-agent.ts` refreshes the Africa, Amsterdam, Munich, and Stockholm recommendations for Vercel Cron.
- `src/pages/api/cron/weekly-seo.ts` refreshes the weekly page keyword plan for Vercel Cron.
- `vercel.json` schedules the daily cron at `0 4 * * *`, which is 06:00 Africa/Johannesburg time, and the weekly cron at `15 4 * * 1`, which is 06:15 every Monday in that timezone.
